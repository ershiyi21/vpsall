...
# 创建emby-crx目录并下载所需文件
echo "开始安装首页大屏海报展示..."
[[ -f PhotoCarousel.js ]] && rm -rf PhotoCarousel.js
wget https://raw.githubusercontent.com/ershiyi21/vpsall/master/emby/PhotoCarousel.js
sed -i '/<\/body>/i\<script src="./PhotoCarousel.js"></script>' index.html
echo "首页大屏海报展示安装完成！"
...
sed -i '/<\/body>/i\<script src="./embyLaunchPotplayer.js"></script>' index.html
echo "第三方播放器插件安装完成！"


#emby 4.7.12.0 mod
read -p "是否开启 emby 4.7.12.0 mod 模式，服务端验证已经开启白名单，故默认否 (y/n)? " confirm
